...
"""
import os,sys,math,time,math,shutil
import subprocess
import optparse
import datetime
import csv
...
def connect_earthexplorer(user, password):
    ...
    params = urllib.parse.urlencode(dict(username=user,password=password, csrf_token=token))
    request = urllib.request.Request("https://ers.cr.usgs.gov/login", params.encode("utf-8"), headers={})
...
def downloadChunks(url,rep,nom_fic):
    ...
    try:
        ...
        if (req.info().get('Content-Type')=='text/html'):
...
def next_overpass(date1,path,sat):
    ...
    """
    date0_L5 = datetime.date(1985,5,4)
    date0_L7 = datetime.date(1999,1,11)
    date0_L8 = datetime.date(2013,5,1)
    if sat=='LT5':
        date0=date0_L5
    elif sat=='LE7':
        date0=date0_L7
...
def check_cloud_limit(imagepath,limit):
    ...
    print('Cloud cover for this scene: ', cloudcover)
    if cloudcover>limit:
...
def find_in_collection_metadata(collection_file,cc_limit,date_start,date_end,wr2path,wr2row):
    ...
    with open(collection_file) as csvfile:
        ...
        for row in reader:
            ...
            acqdate=datetime.date(year_acq,month_acq, day_acq)
...
######################################################################################
##############                main ( NOT USED, KEEP FOR REFERENCE ONLY)           ########################
######################################################################################
