...
class ConvertToWRS:
    ...
    * Landsat WRS-2 Path/Row Shapefiles - download from USGS site
    ...
    def __init__(self, shapefile=None):
        ...
        # Check if the shapefile has been supplied and download if not
        default_shapefilepath = 'tmp/WRS2_descending.shp'
        if shapefile == None:
            if Path(default_shapefilepath).exists():
                shapefile = default_shapefilepath
            else:
                url = "https://landsat.usgs.gov/sites/default/files/documents/WRS2_descending.zip"
                shapefile = 'tmp/WRS2_descending.zip'
                os.mkdir('tmp')
                urllib.request.urlretrieve(url, shapefile)
                with zipfile.ZipFile(shapefile, 'r') as zip_ref:
                    slashpos = shapefile.rfind('/')
                    shapefiledir = shapefile[0:slashpos]
                    zip_ref.extractall(shapefiledir)
                shapefile = default_shapefilepath
